...
const BuyerMessageInbox = () => {
    const userProfile = JSON.parse(localStorage.getItem('userDetails'));
    const userDetails = userProfile.userDetails;
    const [buyersInbox, setBuyersInbox] = useState({});
    ...
    const sendMessage = (data) => {
        ...
        axios.post(environment.API_URL + 'buyer/message/send', data, {headers: {token: userProfile.token}})
        .then(response => {
                        ...
                        <div className="col-8 col-md-8">
                            ...
                            <div className="row d-flex align-items-start">
                                <div className="col-4 col-md-4">
                                    {buyersInbox.jobApplications &&
                                        <div className="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                                            {buyersInbox.jobApplications.map((item) => {
                                                return (
                                                    <button key={item._id} style={{textAlign: 'left'}} className="nav-link mb-2" id={`vl-${item._id}`} data-bs-toggle="pill" data-bs-target={`#v-${item._id}`} type="button" role="tab" aria-controls="v-pills-home" aria-selected="true">{ucwords(item.seller.name)}</button>
                                                )
                                            })
                                ...
                                <div className="col">
                                    ...
                                    <div className="tab-content" id="v-pills-tabContent">
                                        ...
                                        {buyersInbox.jobApplications.length > 0 &&
...
export default BuyerMessageInbox
